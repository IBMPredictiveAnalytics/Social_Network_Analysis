...
class MPDMan(object):
    ...
    def run(self):
	    ...
	    if enableipv6 == "1":
                self.listenRingSock = socket.fromfd(listenRingFD,socket.AF_INET6,socket.SOCK_STREAM)
	    else:
            ...
            if enableipv6 == "1":
                self.mpdSock = socket.fromfd(mpdFD,socket.AF_INET6,socket.SOCK_STREAM)
	    else:
    ...
    def handle_pmi_input(self,sock):
        ...
        if parsedMsg['cmd'] == 'startup_status':
            ...
            if pmiInfo.has_key('host'):
                try:
		    if enableipv6 == "1":
                        toIfhn = socket.getaddrinfo(pmiInfo['host'], 0)[2][4][0]
	            else:
