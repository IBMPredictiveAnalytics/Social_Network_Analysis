...
def mpiexec():
    ...
    try:
	if enableipv6 == "1":
            hostinfo = socket.getaddrinfo(myHost, 0)
	else:
    ...
    if os.environ.has_key('MPD_CON_INET_HOST_PORT'):
        try:
            if enableipv6 == "1":
                myIfhn = socket.getaddrinfo(myHost, 0)[2][4][0]
            else:
...
def collect_args(args,localArgSets):
    ...
    while argidx < len(args)  and  args[argidx] in validGlobalArgs.keys():
        ...
        if garg == '-genv':
            ...
            try:
                if enableipv6 == "1":
                    parmdb[('cmdline',garg)] = socket.getaddrinfo(args[argidx+1], 0)[2][4][0]
                else:
            ...
            try:
                if enableipv6 == "1":
                    hostinfo = socket.getaddrinfo(parmdb['MPIEXEC_IFHN'], 0)
                else:
...
def handle_local_argset(argset,machineFileInfo,msgToMPD):
    ...
    while argidx < len(argset):
        ...
        if argset[argidx] == '-n' or argset[argidx] == '-np':
            ...
            try:
                if enableipv6 == "1":
                    host = socket.getaddrinfo(argset[argidx+1], 0)[2][4][0]
                else:
    ...
    while loRange <= argsetHiRange:
        ...
        if host.startswith('_any_'):
            ...
            try:
                if enableipv6 == "1":
                    msgToMPD['hosts'][asRange] = socket.getaddrinfo(host, 0)[2][4][0]
                else:
        ...
        for node in hostSpec[0].childNodes:
            ...
            for hostname in hostnames:
                if hostname:    # some may be the empty string
                    try:
                        if enableipv6 == "1":
                            ipaddr = socket.getaddrinfo(hostname, 0)[2][4][0]
                        else:
    ...
    for p in procSpec:
        ...
        if p.hasAttribute('host'):
            ...
            if host.startswith('_any_'):
                ...
                try:
                    if enableipv6 == "1":
                        msgToMPD['hosts'][(loRange,hiRange)] = socket.getaddrinfo(host, 0)[2][4][0]
                    else:
