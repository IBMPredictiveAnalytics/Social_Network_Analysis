...
# workaround to suppress deprecated module warnings in python.6
...
import os
enableipv6=os.environ.get("SNAIPV6ENABLE")
...
def mpd_same_ips(host1,host2):    # hosts may be names or IPs
    try:
        if enableipv6 == "1":
            ips1 = socket.getaddrinfo(host1, 0)[2][4][0]    # may fail if invalid host
            ips2 = socket.getaddrinfo(host2, 0)[2][4][0]    # may fail if invalid host
        else:
...
class MPDSock(object):
    def __init__(self,family=socket.AF_INET,socktype=socket.SOCK_STREAM,proto=0,
        ...
        if sock:
            ...
            if enableipv6 == "1" and not family == 1:
                self.sock = socket.socket(family=socket.AF_INET6,type=socktype,proto=proto)
            else:
...
class MPDConListenSock(MPDListenSock):
    def __init__(self,name='console_listen',secretword='',**kargs):
	    ...
	    if enableipv6 == "1":
                sockFamily = socket.AF_INET6    # override above-assigned value
            else:
    ...
    def __init__(self,name='console_to_mpd',mpdroot='',secretword='',**kargs):
        ...
        if mpdroot:
            ...
            if hasattr(socket,'AF_UNIX')  and  sockFamily == socket.AF_UNIX:
                ...
                if conHost:
		    if enableipv6 == "1":
                        conIfhn = socket.getaddrinfo(conHost, 0)[2][4][0]
		    else:
...
class MPDParmDB(dict):
    ...
    def get_parms_from_rcfile(self,parmsToOverride,errIfMissingFile=0):
        ...
        elif hasattr(os,'getuid')  and  os.getuid() == 0 and os.geteuid() == 0:    # if ROOT
            parmsRCFilename = os.path.abspath('/etc/mpd.conf')
        elif os.environ.has_key('HOME'):
            parmsRCFilename = os.path.join(os.environ['HOME'],'.mpd.conf')
            if not os.path.exists(parmsRCFilename):
                parmsRCFilename = os.path.abspath('/etc/mpd.conf')
                print "use /etc/mpd.conf for non root user"
...
class MPDTest(object):
    ...
    def run(self,cmd='',expIn = '',chkEC=0,expEC=0,chkOut=0,expOut='',ordOut=0,
        ...
        if subprocess_module_available:
            ...
            rv['pid'] = runner.pid
            rv['EC'] = runner.wait()
        elif hasattr(popen2,'Popen4'):    # delete when python.4+ is common
