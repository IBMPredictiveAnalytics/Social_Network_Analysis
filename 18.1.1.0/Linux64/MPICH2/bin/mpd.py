...
# fix for ticket #753 where the set() builtin isn't available in python.3
try:
    set
except NameError:
...
class MPD(object):
    def __init__(self):
        ...
        try:
	    if enableipv6 == "1":
                hostinfo = socket.getaddrinfo(self.myHost,0)
                self.myIfhn = hostinfo[2][4][0]    # chgd below when I get the real value
	    else:
    ...
    def get_parms_from_cmdline(self):
        ...
        while argidx < len(sys.argv):
            if sys.argv[argidx] == '--help':
                ...
                try:
		    if enableipv6 == "1":
                        hostinfo = socket.getaddrinfo(ifhn, 0)
                        ifhn = hostinfo[2][4][0]
		    else:
